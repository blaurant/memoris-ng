...
(defn sign-contract
      "Complete step 4: sign one contract (contract-type = :proxywatt | :producer | :sepa)."
      [consumption-repo user-id consumption-id contract-type]
      (let [c         (find-and-check-ownership consumption-repo user-id consumption-id)
            signed-at (str (java.time.Instant/now))
            c'        (consumption/sign-contract c contract-type signed-at)]
        (repo/save! consumption-repo c')))
